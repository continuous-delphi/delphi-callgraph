...
# Creates and pushes a vX.Y.Z release tag for delphi-callgraph.
...
#   - $script:ToolVersion in delphi-callgraph.ps1 matches the Version argument
...
$scriptFile    = Join-Path $repoRoot 'source' 'delphi-callgraph.ps1'
$changelogFile = Join-Path $repoRoot 'CHANGELOG.md'
$tag           = "v$Version"

Write-Host ""
Write-Host "delphi-callgraph  tag-release" -ForegroundColor White
...
if ($scriptContent -notmatch '\$script:ToolVersion\s*=\s*''([^'']+)''') {
  Fail "Could not find '`$script:ToolVersion = ''...''' in delphi-callgraph.ps1."
}

$scriptVersion = $Matches[1]
if ($scriptVersion -ne $Version) {
  Fail "`$script:ToolVersion in delphi-callgraph.ps1 is '$scriptVersion' but -Version arg is '$Version'.`n       Update `$script:ToolVersion in the script and commit before tagging."
}
